...
describe('YeniGorevEkleDuzenle', () => {
  ...
  it('should set status to beklemede when saving in new mode', () => {
    const onSave = vi.fn();
    const onCancel = vi.fn();
    ...
    fireEvent.change(nameInput, { target: { value: 'Yeni Test Görevi' } });
    ...
    expect(onSave).toHaveBeenCalledOnce();
    const savedMission = onSave.mock.calls[0][0];
    expect(savedMission.status).toBe('beklemede');
    expect(savedMission.riskFlags).toEqual([]);
  });
});
